...
def evaluate_query(target_conn, query_text, args, qcache):
    if query_text in qcache:
        ...
        target_conn.execute("PREPARE pp AS " + query_plan.query_text, prepare=False)
        ...
        extract_ou(features)
        qcache[query_plan.query_id] = copy.deepcopy(query_plan_ous)
...
def generate_ou_triggers(target_conn, ou, ougc, qcache):
    trigger_ous = []
    ...
    trigger_ous.append({"node_type": prefix, f"{prefix}_num_triggers": len(ou["ModifyTable_ar_triggers"])})
    trigger_ous.append({"node_type": "AfterTriggerEndQuery", "AfterTriggerEndQuery_num_invoke": len(ou["ModifyTable_ar_triggers"])})
    ...
    for tgoid in ou["ModifyTable_ar_triggers"]:
        ...
        if ou_type == OperatingUnit.ModifyTableInsert:
            ...
            tgquery += " AND ".join([f"{attname} = ${i}" for i, attname in enumerate(trigger_info["attnames"])])
            ...
            tgous = augment_trigger_exec_features(target_conn, tgous, qcache, ougc)
            # Add an InsertUpdateFKTrigger enforcement OU.
            trigger_ous.append({"node_type": OperatingUnit.InsertUpdateFKTriggerEnforce})
...
def augment_trigger_exec_features(target_conn, tgous, qcache, ougc):
    tgous = augment_ous_exec_features(target_conn, None, tgous, qcache, ougc)
    tgous.append({"node_type": OperatingUnit.DestReceiverSPI.name, "DestReceiverSPI_num_output": get_plan_rows_matching_plan_id(tgous, 0)})
    tgous.append({
    ...
    for ou in tgous:
        ...
        elif ou == OperatingUnit.IndexOnlyScan:
            tgous.append({
...
def generate_index_inserts(ou, ougc, num_inserts):
    index_oids = ou["ModifyTable_indexupdates_oid"]
    idx_insert_ous = []
    for _ in range(num_inserts):
        for index_oid in index_oids:
            ...
            if (index_state["num_inserts"] % int((8192.0 / index_state["tuple_len_avg"]) / 2) == 0):
                num_splits = 1
...
def augment_ous_exec_features(target_conn, query, ous, qcache, ougc):
    new_ous = []
    ...
    for ou in ous:
        ...
        if tbl is not None and it_label is not None:
            if query is None or np.isnan(getattr(query, f"{tbl}_hits")):
    ...
    for ou in ous:
        ...
        if ou_type == OperatingUnit.DestReceiverRemote:
            ...
            ou[f"{prefix}_num_defrag"] = int(random.uniform(0, 1) <= (ougc.table_feature_state[tbl]["defrag_percent"] * ou[f"{prefix}_num_iterator_used"]))
            ...
            new_ou = generate_ou_triggers(target_conn, ou, ougc, qcache)
            ...
            for other_ou in ous:
                if other_ou["node_type"] == OperatingUnit.BitmapIndexScan.name and other_ou["plan_node_id"] == ou["left_child_plan_node_id"]:
                    ou["BitmapHeapScan_scan_scanrelid_oid"] = other_ou["BitmapIndexScan_scan_scanrelid_oid"]
            ou["BitmapHeapScan_num_tuples_rows"] = get_plan_rows_matching_plan_id(ous, ou["left_child_node_id"])
...
def generate_query_ous_window(logger, target_conn, ougc, window_index, query_plans, output_dir):
    qcache = {}
    ...
    with target_conn.transaction():
        for query_plan in tqdm(query_plans.itertuples(index=None), total=query_plans.shape[0], leave=False):
            argvals = [f"'{query_plan[col_idx_map[arg]]}'" for arg in args if query_plan[col_idx_map[arg]] is not None]
            query_plan_ous = evaluate_query(target_conn, query.query_text, argvals, qcache)
            query_plan_ous = augment_ous_exec_features(target_conn, query_plan, query_plan_ous, qcache, ougc)
            for ou in query_plan_ous:
                ou["query_id"] = query_plan.query_id
                ou["query_order"] = query_plan.query_order
