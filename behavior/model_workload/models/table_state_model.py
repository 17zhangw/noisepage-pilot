...
class AutoMLTableStateModel():
    ...
    def get_dataset(logger, model_args, inference=False):
        ...
        input_dataset = [{k:item[k] for k in TABLE_STATE_INPUTS + [t[0] for t in TABLE_STATE_TARGETS]} for item in input_dataset]
        inputs = []
        ...
        for i in range(len(input_dataset) - 1):
            ...
            for target, src, _ in TABLE_STATE_TARGETS:
    ...
    def fit(self, dataset):
        ...
        predictor = MultilabelPredictor(labels=[t[0] for t in TABLE_STATE_TARGETS],
    ...
    def inference(self, table_state, table_attr_map, keyspace_feat_space, window, output_df=False):
        def infer(inputs):
            ...
            for target, _, clip in TABLE_STATE_TARGETS:
                if clip == 1:
        ...
        outputs = np.zeros((len(tbl_keys), len(TABLE_STATE_TARGETS)))
        for i, _ in enumerate(tbl_keys):
            for j, (key, _, _) in enumerate(TABLE_STATE_TARGETS):
