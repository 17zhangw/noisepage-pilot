...
##
## FIXME(CONCURRENCY): This is slightly awkward because the block hits are relative
## to the entire query. We'll need to fix this downstream to ensure that we are
## upcharging the block hits for join queries correctly.
#CONCURRENCY_QUERY = """
#SELECT
#    s.window_bucket,
#    s.start_timestamp,
...
def collect_inputs(input_dir, workload_only, psycopg2_conn,
                   work_prefix, buckets, concurrency_steps,
                   slice_window, offcpu_logwidth, gen_exec_features,
                   gen_data_page_features, gen_concurrency_features,
    ...
    with psycopg.connect(psycopg2_conn, autocommit=True, prepare_threshold=None) as connection:
        ...
        if gen_concurrency_features:
            assert False, print("Generating concurrency features is currently disabled.")
